...
class ClickBLE:
    ...
    async def search_for_click(self):
        ...
        self.logger.info(f'Scanning for Click... ({timeout_duration} second timeout)')
        dev = await BleakScanner.find_device_by_name(name="Zwift Click", timeout=timeout_duration)
        if dev:
    ...
    def async_notification_handler(self, characteristic: BleakGATTCharacteristic, data: bytearray):
        ...
        if self.encrypted:
            ...
            if type == const.BATTERY_LEVEL_TYPE:
                ...
                # Construct the relative path to send_string.py
                send_string_path = os.path.abspath(os.path.join(current_dir, "..", "keyboardEmulation", "keyboard", "send_string.py"))
                ...
                if self.button_up_pressed != self.last_button_up_pressed:
                    ...
                    try:
                        if not self.button_up_pressed: subprocess.run([sys.executable, send_string_path, PLUS_KEY])
                    except ImportError:
                ...
                if self.button_down_pressed != self.last_button_down_pressed:
                    ...
                    try:
                        if not self.button_down_pressed: subprocess.run([sys.executable, send_string_path, MINUS_KEY])
                    except ImportError:
                        self.logger.error('Replicating key presses requires root privileges in Linux. Re-run this script as root to get those working')
