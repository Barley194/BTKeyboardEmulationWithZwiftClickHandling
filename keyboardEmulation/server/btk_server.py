...
class BTKbDevice():
    ...
    MY_ADDRESS = "B8:27:EB:84:2A:EA"
    MY_DEV_NAME = "RpiVirtualKeyboard"
    ...
    def listen(self):
        while True: #Keep trying to reconnect
            ...
            try:
                ...
                error("Bluetooth connection error: " + str(err))
                warning("Restarting listener...")
                time.sleep(1)  # Avoid high CPU usage on failures
    ...
    def send_string(self, message):
        try:
            ...
            if err.errno == 107:  # Transport endpoint is not connected
                warning("Connection lost, restarting listener...")
                self.listen()  # Restart the listener
...
class BTKbService(dbus.service.Object):
    ...
    def shutdown(self):
        print("Shutting down: Unregistering profile")
        try:
            self.manager.UnregisterProfile(self.profile_path)
            print("Profile unregistered successfully")
        except dbus.exceptions.DBusException as e:
            print(f"Error unregistering profile: {e}")
